...
public class FenUltils {
	...
	private static Board parseFen(String fenString) {
		final String[] fenBlocks = fenString.trim().split(" ");
		final Builder builder = new Builder();
		final boolean whiteKingSideCastle = whiteKingSideCastle(fenBlocks[2]);
		final boolean whiteQueenSideCastle = whiteQueenSideCastle(fenBlocks[2]);
		final boolean blackKingSideCastle = blackKingSideCastle(fenBlocks[2]);
		final boolean blackQueenSideCastle = blackQueenSideCastle(fenBlocks[2]);
		final String gameState = fenBlocks[0];
		final char[] boardTiles = gameState.replaceAll("/", "")
				.replaceAll("8", "--------")
				.replaceAll("7", "-------")
				.replaceAll("6", "------")
				.replaceAll("5", "-----")
		...
		while (i < boardTiles.length) {
			...
			case 'Q':
				...
				break;
			case 'K':
				// final boolean isCastled = !whiteKingSideCastle && !whiteQueenSideCastle;
			...
			case '-':
				...
				break;
			default:
				throw new RuntimeException("Invalid FEN String " + gameState);
			}
		}
		builder.setMoveMaker(moveMaker(fenBlocks[1]));
